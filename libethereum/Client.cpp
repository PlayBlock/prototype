...
#if BenchMarkFlag
		static BenchMark mark2("Transaction TPS: ");
...
)
{
	...
	m_doRealWork = true;
#if	BenchMarkFlag		
	clog(BenchMarkChannel) << "m_realProduceBlock";
#endif
...
void Client::doWork(bool _doWait)
{
...
#if	BenchMarkFlag
		clog(BenchMarkChannel) << "******resetState: " << worktime.elapsed();
#endif
	}
	...
	if (m_lastProducedNumber != m_working.info().number())
	{
		rejigSealing();
	}
	else {
...
ExecutionResult Client::call(Address const& _from, u256 _value, Address _dest, bytes const& _data, u256 _gas, u256 _gasPrice, BlockNumber _blockNumber, FudgeFactor _ff)
{
	...
	try
	{
		...
		ret = temp.execute(bc().lastBlockHashes(), bc().infoSafe(temp.info().parentHash()), t, Permanence::Reverted);
	}
	catch (...)
	{
