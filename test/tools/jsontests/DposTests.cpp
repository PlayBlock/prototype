...
json_spirit::mObject fillDpBCTest(json_spirit::mObject const& _input)
{
...
void testDpBCTest(json_spirit::mObject const& _o)
{
...
json_spirit::mValue DposBlockTestSuite::doTests(json_spirit::mValue const& _input, bool _fillin) const
{
	g_logVerbosity = 14;
	...
	for (auto const& i : _input.get_obj())
	{
		...
		if (_fillin)
		{
			...
			for (auto& network : test::getNetworks())
			{
				...
				jObjOutput = fillDpBCTest(jObjOutput);
...
//对链进行测试
BOOST_FIXTURE_TEST_SUITE(DposBlockTests, bcTestFixture)
