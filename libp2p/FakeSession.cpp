...
ReputationManager& FakeSession::repMan()
{
	return m_server->repMan();
}

NodeID FakeSession::id() const
{
	return m_peer ? m_peer->id : NodeID();
}

void FakeSession::addRating(int _r)
{
	if (m_peer)
	{
		m_peer->m_rating += _r;
...
bool FakeSession::readPacket(uint16_t _capId, PacketType _t, RLP const& _r)
{
	...
	{
		clog(NetWarn) << "Exception caught in p2p::FakeSession::interpret(): " << _e.what() << ". PacketType: " << _t << ". RLP: " << _r;
...
RLPStream& FakeSession::prep(RLPStream& _s, PacketType _id, unsigned _args)
{
	return _s.append((unsigned)_id).appendList(_args);
}

void FakeSession::sealAndSend(RLPStream& _s)
{
	bytes b;
	_s.swapOut(b);
	send(move(b));
...
void FakeSession::send(bytes&& _msg)
{
...
void FakeSession::drop(DisconnectReason _reason)
{
	if (m_dropped)
		return;
...
void FakeSession::disconnect(DisconnectReason _reason)
{
...
void FakeSession::registerCapability(CapDesc const& _desc, std::shared_ptr<Capability> _p)
{
	DEV_GUARDED(x_framing)
	{
