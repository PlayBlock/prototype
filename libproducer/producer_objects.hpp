...
namespace config = ::dev::eth::config;
namespace chain = ::eos::chain;
namespace types = ::dev::types;

using dev::eth::chain::ProducerRound;
...
class ProducerVotesObject : public chainbase::object<chain::producer_votes_object_type, ProducerVotesObject> {
   ...
   std::pair<types::ShareType,id_type> getVoteOrder()const { return { race.speed, id }; }
   ...
   struct {
      ...
      void update(types::ShareType currentSpeed, types::UInt128 currentPosition, types::UInt128 currentRaceTime) {
		 ...
		 if (currentRaceTime > std::numeric_limits<types::UInt128>::max() - projectedTimeToFinish)
		 {
			 cwarn << "Producer race time has overflowed"
				 << "currentTime: " << currentRaceTime << " "
				 << "timeToFinish: " << projectedTimeToFinish << " "
				 << "limit: " << std::numeric_limits<types::UInt128>::max();
			 BOOST_THROW_EXCEPTION(std::overflow_error("Producer race time has overflowed"));
		 }
...
class ProducerScheduleObject : public chainbase::object<chain::producer_schedule_object_type, ProducerScheduleObject> {
    ...
    */
   ProducerRound calculateNextRound(chainbase::database& db) const;
